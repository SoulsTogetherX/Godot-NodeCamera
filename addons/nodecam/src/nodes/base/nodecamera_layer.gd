...
## or a [NodeCameraGroup], typically caused from being added to the
## scenetree.
## [br][br]
## Also see [signal deactivated], and [member disabled].
signal activated
## Emitted when the layer was previously registered, but later unregistered
## from [NodeCameraManager] or a [NodeCameraGroup], typically caused from
## being removed from the scenetree.
## [br][br]
...
## Controls the order [NodeCameraLayer] nodes are processed in, when relevant.
...
## [br][br]
## Also see [signal activated] and [signal deactivated].
...
## [br][br]
## [b]NOTE[/b]: This [NodeCameraLayer] is only processed by [NodeCameraHost] nodes
## that share one or more layers, checked via a bitwise 'and' operation.
...
## Returns the current scope of the active
## [color=#D6D000][b]Runtime Method[/b][/color].
## [br][br]
...
## array of [NodeCameraExecutionScope] resources parents. If [code]null[/code],
...
## [NodeCameraExecutionScope] resources parents, is found. If an empty list
...
func get_closest_active_layer_list() -> Array[NodeCameraLayer]:
	...
	while layer != null:
		...
		if (
			!layer._parent_group ||
			!NodeCameraUtility.vaild_route(layer._parent_group, layer)
		):
			return []
...
## Forces all active [NodeCameraExecutionScope] resources to advance any [LayerRecord]
## resources, featuring this [NodeCameraStaged], one stage forward.
## [br][br]
...
## Forces all active [NodeCameraExecutionScope] resources to overwrite the stage
## of any [LayerRecord] resources, featuring this [NodeCameraStaged], assuming
## it's current stage is before [param stage].
## [br][br]
## Stages go in the order of [code]STARTING > RUNNING > ENDING > HALTED[/code
## [br][br]
## Also see [method NodeCameraLayer.get_parent_scopes],
## [method overwrite_stage], and
## [enum NodeCameraUtility.LAYER_STAGES].
...
## Returns the current [NodeCameraGroup] running this layer. If [code]null[/code]
