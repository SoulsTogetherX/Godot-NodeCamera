...
#region Private
var _master_manager := NodeCamera2DLayerManager.new()
...
@export var active : bool = true:
	set = set_active,
	get = get_active
## Sets a mask that [NodeCamera2DHost] will use filter out unneeded layers.
...
func _settup_group() -> void:
	_is_in_group = (get_parent() is NodeCamera2DGroup)
func _settup_registry() -> void:
	...
	if !top_level:
		...
		if parent is NodeCamera2DGroup:
			_is_top_level = false
	...
	_master_manager = NodeCamera2DManager.get_layer_manager()
	_master_manager.register_layer(self)
#endregion
...
## or stopped.
## [br][br]
## Also see [method NodeCamera2DEffect.effect_tick],
## [method NodeCamera2DTransition.transition_tick], and [signal tick_state_changed].
...
## Returns [code]true[/code] if this node is in a child of a [NodeCamera2DGroup] node.
...
## or this layer is not a child of a [NodeCamera2DGroup] node.
